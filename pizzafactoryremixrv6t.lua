...
while gui.Parent do
	...
	local function FindPizzaSlicer(parent)
		warn("FindPizzaSlicer called on line 1")
		if not parent then
			warn("Parent is nil on line 3")
			return nil
		end
	
		local ps = {}
		warn("ps table created on line 7")
		local children = parent:GetChildren()
		warn("children table created on line 9")
	
		-- First, look for the Pizza Slicer in the given parent.
		for i = 1, #children do
			local s = children[i]
			warn("checking child on line 14")
			if s:IsA("Tool") and s.Name == "Pizza Slicer" then
				warn("Pizza Slicer found in parent on line 16")
				ps[#ps + 1] = s
			end
		end
		...
		if #ps == 0 then
			warn("ps table is empty on line 22")
			-- Teleport if the player is too far.
			HandlePosition(CFrame.new(58.74, 3.80, 12.40)) 
			wait(0.05)
	
			-- Look for the Pizza Slicer in the workspace.
			local pizzaSlicer = workspace:FindFirstChild("Pizza Slicer")
			warn("checking workspace for Pizza Slicer on line 28")
			if pizzaSlicer then
				warn("Pizza Slicer found in workspace on line 30")
				ps[#ps + 1] = pizzaSlicer
			else
				warn("Pizza Slicer not found in workspace on line 33")
				-- Look for the Pizza Slicer in the player's backpack or character.
				pizzaSlicer = player.Backpack:FindFirstChild("Pizza Slicer") or player.Character:FindFirstChild("Pizza Slicer")
				warn("checking player's backpack and character for Pizza Slicer on line 37")
				if pizzaSlicer then
					warn("Pizza Slicer found in player's backpack or character on line 39")
					humanoid:EquipTool(pizzaSlicer)
					wait(0.05)
				else
					warn("Pizza Slicer not found in player's backpack or character on line 43")
				end
			end
	
			-- Check again in the parent if not found in the workspace
			if #ps == 0 then
				warn("ps table is still empty on line 48")
				for _, tool in pairs(parent:GetChildren()) do
					warn("checking child again on line 50")
					if tool:IsA("Tool") and tool.Name == "Pizza Slicer" then
						warn("Pizza Slicer found in parent again on line 52")
						ps[#ps + 1] = tool
					end
				end
			end
		...
		warn("returning ps[1] on line 59")
		return ps[1]
	end
