...
local settings = {refill_at=45, refill_end=60, deliver_at=8, stay_in_kitchen=true}
			...
			for k, v in next, getupvalues(f) do
				if typeof(v) == "Instance" then
					if v.Name == "CashOut" then
						-- Check if the type matches before assigning
						if type(getupvalues(f)[k]) == type({MouseButton1Click = {wait = function() end, Wait = function() end}}) then
							setupvalue(f, k, {MouseButton1Click = {wait = function() end, Wait = function() end}})
						else
							warn("Type mismatch prevented on 'CashOut'")
						end
					end
				end
			end
...
titleLbl=Create("TextLabel",topbar,{Name="titleLbl", TextWrapped=true, Size=UDim2.new(0.5,0,1,0), Text="Pizza Factory Remix V7", TextSize=14, Font="GothamSemibold", 
...
cashierBtn=Create("ImageButton",cashier,{Name="cashierBtn", ImageTransparency=1, BorderSizePixel=0, Size=UDim2.new(0.38,0,1,0), BackgroundColor3=Color3.new(0.333,0.333,0.333)})
...
Label_5=Create("TextLabel",supplier,{TextWrapped=true, Size=UDim2.new(0.6,0,1,0), Text="Supplier", TextSize=14, TextXAlignment="Left", Font="SourceSans", 
	BackgroundTransparency=1, Position=UDim2.new(0.4,0,0,0), TextColor3=Color3.new(1,1,1), TextScaled=true, BackgroundColor3=Color3.new(1,1,1)})
supplierBtn=Create("ImageButton",supplier,{Name="supplierBtn", ImageTransparency=1, BorderSizePixel=0, Size=UDim2.new(0.38,0,1,0), BackgroundColor3=Color3.new(0.333,0.333,0.333)})
...
boxerLbl=Create("TextLabel",boxer,{TextWrapped=true, Size=UDim2.new(0.6,0,1,0), Text="Boxer", TextSize=14, TextXAlignment="Left", Font="SourceSans", 
	BackgroundTransparency=1, Position=UDim2.new(0.4,0,0,0), TextColor3=Color3.new(1,1,1), TextScaled=true, BackgroundColor3=Color3.new(1,1,1)})
boxerBtn=Create("ImageButton",boxer,{Name="boxerBtn", ImageTransparency=1, BorderSizePixel=0, Size=UDim2.new(0.38,0,1,0), BackgroundColor3=Color3.new(0.333,0.333,0.333)})
...
allOffBtn.InputBegan:Connect(function()
	if game:GetService("UserInputService"):IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
		...
		if toggleAllSlider.Position.X.Scale<.01 then
			...
			cashierBtn.BackgroundColor3 = Color3.new(0.333,0.333,0.333)
			cookBtn.BackgroundColor3 = Color3.new(0.333,0.333,0.333)
			boxerBtn.BackgroundColor3 = Color3.new(0.333,0.333,0.333)
			deliveryBtn.BackgroundColor3 = Color3.new(0.333,0.333,0.333)
			supplierBtn.BackgroundColor3 = Color3.new(0.333,0.333,0.333)
	end
end)
...
local function smoothTP2(cf)
	local cf0 = (cf - cf.p) + root.Position + Vector3.new(0, 2.75, 0)
	...
	bodyVelocity.MaxForce = Vector3.new(0, 9999, 0)
	...
	for i = 0, diff.Magnitude, 0.7625 do
		humanoid.Sit = false
	...
	for i=1,#children do
		...
		if c:FindFirstChild("HumanoidRootPart") then
			...
			elseif ffc(c,"Head") and ffc(c,"Humanoid") and c.Head.CFrame.Z<125 and ((c.Humanoid.SeatPart and c.Humanoid.SeatPart.Anchored) or (c.Humanoid.SeatPart==nil and (c.Head.Velocity.Z^2)^.5<.0001)) then
				...
				wait(0.3)
				pcall(function()
		    		if (root.Position-Vector3.new(48.30, 3.60, 91.15)).magnitude>3.1 then smoothTP(CFrame.new(48.30, 3.60, 91.15)) wait(0.2) end
				wait()
...
while gui.Parent do
	...
	for zz=1,3 do
		if doCashier then
			...
			if c and order then
				...
				if (root.Position-Vector3.new(50.30, 3.80, 83.24)).magnitude>8 then smoothTP(CFrame.new(50.30, 3.80, 83.24)) wait(0.2) end
	...
	if doDelivery then
		...
		if #wstools > 1 or (wstools[1] and ffc(wstools[1].Handle,"X10")) then
			-- Table Pickup.
			if (root.Position-Vector3.new(54.45, 4.02, -15)).magnitude>9 then smoothTP(CFrame.new(54.45, 4.02, -15)) wait(0.2) end
			wait()
			for i=1,#wstools do
