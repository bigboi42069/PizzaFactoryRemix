...
titleLbl=Create("TextLabel",topbar,{Name="titleLbl", TextWrapped=true, Size=UDim2.new(0.5,0,1,0), Text="Pizza Factory Remix V9", TextSize=14, Font="GothamSemibold", 
	...
	for i=1,#children do
		...
		if c:FindFirstChild("HumanoidRootPart") then
			...
			elseif ffc(c,"Head") and ffc(c,"Humanoid") and c.Head.CFrame.Z<125 and ((c.Humanoid.SeatPart and c.Humanoid.SeatPart.Anchored) or (c.Humanoid.SeatPart==nil and (c.Head.Velocity.Z^2)^.5<.0001)) then
				...
				wait(0.3)
				pcall(function()
		    		if (root.Position-Vector3.new(47.25, 6.85, 91.24)).magnitude>2.5 then smoothTP(CFrame.new(47.25, 3.3, 91.24)) wait(0.2) end
				wait()
