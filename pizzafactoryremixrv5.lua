...
-- Helper function to find the pizza cutter tool
...
local function cutPizza(pizza)
    ...
    if not pizzaCutter then
        -- Try to get a pizza cutter if not found
    ...
    -- Equip the pizza cutter if it's not already equipped
    ...
    wait(0.05) -- Wait for the cutting animation to complete
...
-- Main loop for boxing and cutting pizza
for zz = 1, 7 do
    if doBoxer then
        didsomething = false
        ...
        -- Handle delivery box (boxD) when found
        ...
        -- Handle closed box (to be opened)
        ...
        -- Handle pizza boxing (if there's an open box and pizza)
        if openBox and boxP then
            ...
            if cutPizza(boxP) then
                -- Anchor and position pizza in the box
        ...
        -- Delay between actions or break if nothing was done
