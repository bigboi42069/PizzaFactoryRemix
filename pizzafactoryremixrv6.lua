	...
	for i=1,#children do
		...
		if c:FindFirstChild("HumanoidRootPart") then
			...
			elseif ffc(c,"Head") and ffc(c,"Humanoid") and c.Head.CFrame.Z<130 and ((c.Humanoid.SeatPart and c.Humanoid.SeatPart.Anchored) or (c.Humanoid.SeatPart==nil and (c.Head.Velocity.Z^2)^.5<.0001)) then
				wait(0.05)
				pcall(function()
		    		if (root.Position-Vector3.new(49.00, 7.20, 91.00)).magnitude>1 then smoothTP(CFrame.new(49.00, 3.55, 91.00)) wait(0.05) end
...
while gui.Parent do
	...
	if doDelivery then
		...
		if #wstools > 1 or (wstools[1] and ffc(wstools[1].Handle,"X10")) then
			-- Table Pickup.
			if (root.Position-Vector3.new(54.45, 4.02, -15)).magnitude>9 then smoothTP(CFrame.new(54.45, 4.02, -15)) wait(0.02) end
			for i=1,#wstools do
