	...
	for zz=1,7 do
		if doBoxer then
			...
			local function cutPizza(pizza)
                -- Equip the pizza cutter tool
                local pizzaCutter = character:FindFirstChild("PizzaCutter")
                if not pizzaCutter then
                    -- If the character doesn't have a pizza cutter, try to get one
                    network:FireServer("GetTool", "PizzaCutter")
                    wait(0.5) -- Wait for the tool to be equipped
                    pizzaCutter = character:FindFirstChild("PizzaCutter")
                end
                ...
                if pizzaCutter then
                    ...
                    wait(1) -- Wait for the cutting animation to complete
                else
                    warn("Failed to equip pizza cutter")
                    local h = Instance.new("Hint",workspace)
	                h.Text = "Level 8 Executor Required."
	                wait(3)
	                h:Destroy()
